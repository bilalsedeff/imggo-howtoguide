"""
Simple Document Classification Test
...
def main():
    print("="*60)
    print("TESTING DOCUMENT CLASSIFICATION PATTERN")
    print("="*60)
    ...
    pattern_id = os.getenv("DOCUMENT_CLASS_PATTERN_ID")
    if not pattern_id:
    ...
    # Use document classification image
    ...
    try:
        client = ImgGoClient()
        print("Processing document for classification...")
        ...
        # JSON format output
        output_file = outputs_dir / "classification_output.json"
        try:
            class_data = result if isinstance(result, dict) else json.loads(result)
