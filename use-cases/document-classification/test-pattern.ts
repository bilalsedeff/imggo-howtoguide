/**
 * Document Classification Test - TypeScript/Node.js Example
...
function getPatternId(): string | null {
    const envId = process.env.DOCUMENT_CLASS_PATTERN_ID;
    ...
    return new Promise((resolve, reject) => {
        ...
        const options: https.RequestOptions = {
            ...
            headers: {
                ...
                'Idempotency-Key': `doc-class-ts-${Date.now()}`
            }
        };
...
async function main(): Promise<void> {
    ...
    console.log('TESTING DOCUMENT CLASSIFICATION PATTERN (TypeScript)');
    console.log('='.repeat(60));
    ...
    if (!patternId) {
        console.log('\nX Error: DOCUMENT_CLASS_PATTERN_ID not set');
        console.log('Run create-pattern.py first to create a pattern');
        process.exit(1);
    }

    const testImage = path.join(__dirname, '../../examples/test-images/document-classification1.png');
    ...
    console.log(`Test Image: document-classification1.png\n`);

    try {
        console.log('Processing document for classification...');
        ...
        const outputFile = path.join(outputDir, 'classification_ts_output.json');
